<template>
  <el-tabs type="border-card">
    ...
    <el-tab-pane label="各科成绩平均分">
      <el-form-item label="选择省份：" class="select-container">
        <el-select v-model="selectedProvince2" placeholder="请选择省份" @change="updateLable2">
    ...
    <el-tab-pane label="具体年份各科成绩平均分">
      <el-form-item label="选择省份：" class="select-container">
        <el-select v-model="selectedProvince4" placeholder="请选择省份" @change="updateLable4">
          <el-option v-for="province in provinces" :key="province" :label="province" :value="province"></el-option>
        </el-select>
      </el-form-item>
...
const subject_avg_score_by_area = ref()
const subjectAvgScores = ref()
//声明周期函数，自动执行初始化
onMounted(async () => {
  await initLable1();
  await initLable2();
...
const initLable1 = async () => {
  try {
    // 从服务器获取数据
    const response = await proxy.$http.get('/scorestatistics/area_average_score_trend/');
    const area_average_score_trend = response.data.area_average_score_trend;

    // 解析数据，获取年份和各省份的平均分
    const years = area_average_score_trend[Object.keys(area_average_score_trend)[0]].map(item => item.year);
    const provinces = Object.keys(area_average_score_trend);
    const scoresByProvince = provinces.map(province => area_average_score_trend[province].map(item => parseFloat(item.average_score).toFixed(2)));
...
const initLable2 = async () => {
  const response = await proxy.$http.get('/scorestatistics/subject_avg_score_by_area/');
  subject_avg_score_by_area.value = response.data.subject_avg_score_by_area;
  provinces.value = Object.keys(subject_avg_score_by_area.value);
...
const updateLable2 = () => {
  ...
  const years = Object.keys(subject_avg_score_by_area.value[selectedData]).map((year) => year);
  const firstYearData = subject_avg_score_by_area.value[selectedData][Object.keys(subject_avg_score_by_area.value[selectedData])[0]];
  const subjects = Object.keys(firstYearData);
  console.log(subject_avg_score_by_area.value[selectedData])
  // 构建数据集对象
  const dataset = {
    dimensions: ['year', ...subjects], // 维度包括年份和所有科目
    //@ts-ignore
    source: Object.keys(subject_avg_score_by_area.value[selectedData]).map(year => {
      const item = subject_avg_score_by_area.value[selectedData][year];
      return {
        year: year,
...
const initLabel3 = async () => {
  try {
    ...
    const response = await proxy.$http.get('/scorestatistics/area_pass_rate_trend/');
    const area_pass_rate_trend = response.data.area_pass_rate_trend;
    ...
    for (const province in area_pass_rate_trend) {
      //@ts-ignore
      legendData.push(province);
      const passRateData = [];

      area_pass_rate_trend[province].forEach(entry => {
        //@ts-ignore
...
const initLable4 = async () => {
  const response = await proxy.$http.get('/scorestatistics/subject_avg_score_by_area_and_year/');
  subjectAvgScores.value = response.data.subject_avg_score_by_area_and_year;
...
const updateLable4 = async () => {
  ...
  const provinceData = subjectAvgScores.value[selectedProvince4.value];
